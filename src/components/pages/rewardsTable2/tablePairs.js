...
const TableRow = ({ item, isLast, idx, isMigration }) => {
  ...
  return (
    ...
    >
      ...
      <div className='flex flex-col w-1/2 mt-2 lg:mt-0 web-table-row lg:w-[12%] justify-center'>
      ...
      {/* second row */}
      <div className={`${isOpen ? 'flex' : 'hidden'} lg:flex flex-col web-table-row w-1/2 mt-3 lg:mt-0 lg:w-[18%] justify-center`}>
        <p className='lg:hidden text-sm f-f-fg font-medium text-table-row-title'>My Stake</p>
        <div className='web-table-row'>
          <div> {formatAmount(item.account.staked0)} {item.token0.symbol}</div>
          <div> {formatAmount(item.account.staked1)} {item.token1.symbol}</div>
        </div>
      </div>
              ...
              <div className='space-x-5 xl:space-x-6 w-full flex items-center lg:justify-end'>
                {fusion && (
                  <TransparentButton
                    ...
                    className='border-img h-10 px-[26px] text-grey flex whitespace-nowrap flex-col items-center justify-center tex-sm xl:text-[17px] w-1/2'
                  />
                )}
                {gamma && (
                  <TransparentButton

                    content={'Migrate to V3'}
                    onClickHandler={() => {
                      setMigrateV3(true)
                    }}
                    fontWeight={'font-medium'}
          ...
          {pairsData.slice(currentPage * pageSize, (currentPage + 1) * pageSize).length > 0 ? (
            <>
              <Sticky
                ...
                className={`z-[100]`}
              >
                <div className='w-[18%] font-medium text-[17px] xl:text-[18px] normal-td-header f-f-fg'></div>
                {sortOptions.map((option, index) => (
                  <div
                    className={`${index === 1 ? 'w-[18%]' : index === 0 ? 'w-[12%]' : 'w-[18%]'} font-medium text-[17px] xl:text-[18px] normal-td-header f-f-fg`}
                    key={`header-${index}`}
                  >
                    <div
                      onClick={() => {
