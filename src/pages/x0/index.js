...
import {
    useJaceFunContractV2, useTokenFactoryContractV2, useAbiContract
} from "../../hooks/useContract";


import { getContract } from '../../utils/contract'
import { getJsonRpcContract } from '../../utils/contractHelpers'
...
import {getTokenFactoryAddressV2} from "../../utils/addressHelpers";
...
const X0 = () => {
    ...
    const jaceFunContract = useJaceFunContractV2()
    const tokenFactoryContract = useTokenFactoryContractV2()
    ...
    const { onSell,pending } = useAddLiquidity()

    const tokenFactoryAddress = getTokenFactoryAddressV2();
// 方法1: 使用Math.floor() - 向下取整（适用于正数）
    ...
    const setPercent = (percent, type) => {
        if (type === "buy") setBuyAmount( floorDecimal(okbBalance * percent / 100.0,4));
    ...
    const handleDomainChange =  (e) => {
        ...
        if(domainInput.toLocaleUpperCase() == "GDOG" ||
            domainInput.toLocaleUpperCase() == "GCAT" ){
            status.innerText = 'The token has been issued';
            setNameStatus(status.innerText)
            document.getElementById('tokenDetails').style.display = 'none';
            //toast.error(status.innerText, { transition: Zoom });
            return;
    ...
    const createDomainToken = async () => {
        ...
        let domainInput = tokenId.trim();
        if(domainInput.toLocaleUpperCase() == "GDOG" ||
            domainInput.toLocaleUpperCase() == "GCAT" ){
            setCreateDomainStatus( 'The token has been issued');
            return;
        }
                        ...
                        <label className="font-13 text-error-red">{nameStatus}</label>
                            ...
                            <div className="section">
                                ...
                                <label className="font-13">
                                    Amount ({NetworksData[window.currChainId] ? NetworksData[window.currChainId].nativeCurrency.symbol :''}):{" "}
